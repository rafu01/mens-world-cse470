...
public class CartController {
    ...
    public RedirectView add_to_cart(@PathVariable int id, Model model, Principal principal, HttpSession session,
        ...
        if (cart == null) {
            ...
            if (available >= 1)
                ...
                // session.setAttribute("message",new Message("product sold out","notification
                // is-danger"));
    ...
    public RedirectView increment_quantity(@PathVariable int id, Model model, Principal principal,
            HttpSession session) {
        Cart cart = (Cart) session.getAttribute("cart");
        for (Pair pair : cart.getProducts()) {
    ...
    public RedirectView decrement_quantity(@PathVariable int id, Model model, Principal principal,
            HttpSession session) {
        Cart cart = (Cart) session.getAttribute("cart");
        for (Pair pair : cart.getProducts()) {
            if (pair.getProduct().getId() == id) {
    ...
    public String add_coupon(@RequestParam(required = false) String coupon, Model model, Principal principal,
            HttpSession session) {
        Cart cart = (Cart) session.getAttribute("cart");
        ...
        for (Shop shop : available_coupons.keySet()) {
            for (Coupon coup : available_coupons.get(shop)) {
                ...
                if (coup.getName().equals(coupon)) {
                    double price_after_discount = cart.getTotal_after_discount(shop, products, coup.getPercentage());
